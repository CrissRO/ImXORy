...
class Nor(Gate):
    ...
    def __init__(self,parentMap,fromData = False):
        print("NOR")
        Gate.__init__(self,2,parentMap,Color.lime,"NOR",ImageLoader.NOR,fromData)
        if not fromData:
            self.setAt()
    ...
    def calculate(self):
        if self.outputs != None:
            if self.inputs[0].value == self.inputs[1].value and self.inputs[0].value == 1:
                self.outputs.value = 0
            else:
